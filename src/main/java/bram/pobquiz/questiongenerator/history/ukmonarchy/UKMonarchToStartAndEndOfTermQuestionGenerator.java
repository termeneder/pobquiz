...
public class UKMonarchToStartAndEndOfTermQuestionGenerator {

	private UKMonarchs c_ukMonarchs;
	private QuestionList c_questionList;
	
	public UKMonarchToStartAndEndOfTermQuestionGenerator(UKMonarchs ukMonarchs,
	...
	private void addFactory(Monarch monarch, List<QuestionFactory> factoryList) {
		if (monarch.beginTerm == null || monarch.endTerm == null) {
			return;
		}
		...
		if (sameFactory == null) {
			...
			newFactory.withAnswer(Integer.toString(monarch.endTerm));
			newFactory.withCaterorgy("UK");
			newFactory.withCaterorgy("UK Monarchs");
			newFactory.withCaterorgy("Monarch to start and end of term");
			factoryList.add(newFactory);
		} else {
			sameFactory.withAnswer(Integer.toString(monarch.beginTerm));
			sameFactory.withAnswer(Integer.toString(monarch.endTerm));
		}
