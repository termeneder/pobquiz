...
public class StartAndEndOfTermToUKMonarchQuestionGenerator {

	private UKMonarchs c_ukMonarchs;
	private QuestionList c_questionList;
	
	public StartAndEndOfTermToUKMonarchQuestionGenerator(UKMonarchs ukMonarchs,
	...
	private void addFactory(Monarch monarch, List<QuestionFactory> factoryList) {
		...
		if (sameFactory == null) {
			...
			newFactory.withCaterorgy("Start and end of term to monarch");
	...
	private String makeQuestion(Monarch monarch) {
		...
		question += " of " + monarch.monarch_of + " from " + monarch.beginTerm + " to " + monarch.endTerm + "?";
		return question;
	}
