...
public class LifeToPresidentQuestionGenerator {

	private USPresidents c_usPresidents;
	private QuestionList c_questionList;
	
	public LifeToPresidentQuestionGenerator(USPresidents usPresidents,
	...
	private void addQuestion(President president) {
		if (president.birth != null && president.death != null) {
			QuestionFactory factory = new QuestionFactory();
			
			factory.withQuestion("Which US president lived from " + president.birth + " to " + president.death + "?");
	
			factory.withAnswer(president.name);
			factory.withCaterorgy("US");
			factory.withCaterorgy("US Presidents");
			factory.withCaterorgy("Life to US President");
